...
func Info(buf []byte, o ImageOptions) (Image, error) {
	...
	if err != nil {
		return image, Wrap(KindProcessing, "Cannot retrieve image metadata", err)
	}
...
func Resize(buf []byte, o ImageOptions) (Image, error) {
	if o.Width == 0 && o.Height == 0 {
		return Image{}, New(KindInvalidParam, "Missing required param: height or width")
	}
...
func Fit(buf []byte, o ImageOptions) (Image, error) {
	if o.Width == 0 || o.Height == 0 {
		return Image{}, New(KindInvalidParam, "Missing required params: height, width")
	}
	...
	if dims.Width == 0 || dims.Height == 0 {
		return Image{}, New(KindUnsupportedMedia, "Width or height of requested image is zero")
	}
...
func Extract(buf []byte, o ImageOptions) (Image, error) {
	if o.AreaWidth == 0 || o.AreaHeight == 0 {
		return Image{}, New(KindInvalidParam, "Missing required params: areawidth or areaheight")
	}
...
func Rotate(buf []byte, o ImageOptions) (Image, error) {
	if o.Rotate == 0 {
		return Image{}, New(KindInvalidParam, "Missing required param: rotate")
	}
...
func Thumbnail(buf []byte, o ImageOptions) (Image, error) {
	if o.Width == 0 && o.Height == 0 {
		return Image{}, New(KindInvalidParam, "Missing required params: width or height")
	}
...
func Zoom(buf []byte, o ImageOptions) (Image, error) {
	if o.Factor == 0 {
		return Image{}, New(KindInvalidParam, "Missing required param: factor")
	}
	...
	if o.Top > 0 || o.Left > 0 {
		if o.AreaWidth == 0 && o.AreaHeight == 0 {
			return Image{}, New(KindInvalidParam, "Missing required params: areawidth, areaheight")
		}
...
func Convert(buf []byte, o ImageOptions) (Image, error) {
	if o.Type == "" {
		return Image{}, New(KindInvalidParam, "Missing required param: type")
	}
	if ImageType(o.Type) == bimg.UNKNOWN {
		return Image{}, New(KindInvalidParam, "Invalid image type: "+o.Type)
	}
	opts := BimgOptions(o)
...
func Watermark(buf []byte, o ImageOptions) (Image, error) {
	if o.Text == "" {
		return Image{}, New(KindInvalidParam, "Missing required param: text")
	}
...
func WatermarkImage(buf []byte, o ImageOptions) (Image, error) {
	if len(o.ImageBytes) == 0 {
		return Image{}, New(KindInvalidParam, "Missing required param: image")
	}
...
func GaussianBlur(buf []byte, o ImageOptions) (Image, error) {
	if o.Sigma == 0 && o.MinAmpl == 0 {
		return Image{}, New(KindInvalidParam, "Missing required param: sigma or minampl")
	}
	opts := BimgOptions(o)
	return Process(buf, opts)
}
...
func Pipeline(buf []byte, o ImageOptions) (Image, error) {
	if len(o.Operations) == 0 {
		return Image{}, New(KindInvalidParam, "Missing or invalid pipeline operations JSON")
	}
	if len(o.Operations) > 10 {
		return Image{}, New(KindInvalidParam, "Maximum allowed pipeline operations exceeded")
	}

	for i, operation := range o.Operations {
		var exists bool
		if operation.Operation, exists = OperationsMap[operation.Name]; !exists {
			return Image{}, New(KindInvalidParam, fmt.Sprintf("Unsupported operation name: %s", operation.Name))
		}
