...
func TestExtractImageTypeFromMime(t *testing.T) {
	...
	for _, file := range files {
		if image.ExtractImageTypeFromMime(file.mime) != file.expected {
...
func TestIsImageTypeSupported(t *testing.T) {
	...
	for _, file := range files {
		if image.IsImageMimeTypeSupported(file.name) != file.expected {
...
func TestImageType(t *testing.T) {
	...
	for _, file := range files {
		if image.ImageType(file.name) != file.expected {
...
func TestGetImageMimeType(t *testing.T) {
	...
	for _, file := range files {
		if image.GetImageMimeType(file.name) != file.expected {
